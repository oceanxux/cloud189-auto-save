...
interface Task { id: number; resourceName: string; currentEpisodes: number; totalEpisodes: number; lastFileUpdateTime: string; account: Account; enableLazyStrm: boolean; lastOrganizeError?: string; realFolderName?: string; }
interface ProcessedRecord { id: number; sourceFileName: string; sourceMd5?: string; status: string; updatedAt: string; }
...
const TaskTab: React.FC<TaskTabProps> = ({ onCreateTask, onShowToast }) => {
  ...
  const handleDeleteRecords = async (ids: number[]) => {
    ...
    if (!confirm(`确定要删除选中的 ${ids.length} 条处理记录吗？\n删除后系统将不再跳过这些文件的转存。`)) return;
            ...
            <div className="flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-4 shrink-0">
                <div className="text-right hidden sm:block">
                  ...
                  <div className="w-20 h-1 bg-slate-100 dark:bg-slate-800 rounded-full overflow-hidden"><div className="h-full bg-blue-500" style={{ width: `${task.totalEpisodes > 0 ? (task.currentEpisodes / task.totalEpisodes) * 100 : 0}%` }} /></div>
                </div>
              ...
              <div key={record.id} className={`p-3 bg-[var(--bg-main)] rounded-xl border transition-all ${selectedRecordIds.includes(record.id) ? 'border-blue-500 ring-1 ring-blue-500/20 bg-blue-50/5' : 'border-[var(--border-color)]'}`}>
                <div className="flex justify-between items-start gap-4">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    <span className={`px-2 py-0.5 rounded text-[9px] font-black ${record.status === 'success' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-red-500/10 text-red-500'}`}>{record.status === 'success' ? '成功' : '失败'}</span>
